...
import { getFarmDecimals } from '../constants/farm'
...
const getPoolChartData = async poolid => {
  ...
  let decimals = getFarmDecimals(poolid)
  try {
    let allFound = false
    let skip = 0
    while (!allFound) {
    ...
    data.forEach((dayData, i) => {
      ...
      newdata.push({
        date: dayData.date,
        totalToken: getBalanceNumber(dayData.totalToken, decimals),
      })
    })
    ...
    if (newdata[0]) {
      ...
      while (timestamp < utcEndTime.unix() - oneDay) {
        ...
        if (!dayIndexSet.has(currentDayIndex)) {
          ...
          totalToken = getBalanceNumber(dayIndexArray[index].totalToken, decimals)
          index = index + 1
        }
        timestamp = nextDay
      }
