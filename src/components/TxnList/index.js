...
const DashGrid = styled.div`
  ...
  @media screen and (min-width: 1080px) {
    ...
    grid-template-areas: 'txn value  account time';
  }
`
...
function TxnList({ transactions, symbol0Override, symbol1Override, color }) {
  ...
  useEffect(() => {
    if (transactions && transactions.mints && transactions.burns) {
      let newTxns = []
      if (transactions.mints.length > 0) {
        transactions.mints.map(mint => {
          ...
          newTxn.token0Amount = mint.amount
          newTxn.token1Amount = mint.amount
          newTxn.account = mint.address
          newTxn.token0Symbol = mint.poolid
          newTxn.token1Symbol = mint.poolid
          newTxn.amountUSD = mint.amount
      ...
      if (transactions.burns.length > 0) {
        transactions.burns.map(burn => {
          ...
          newTxn.token0Amount = burn.amount
          newTxn.token1Amount = burn.amount
          newTxn.account = burn.address
          newTxn.token0Symbol = burn.poolid
          newTxn.token1Symbol = burn.poolid
          newTxn.amountUSD = burn.amount
          return newTxns.push(newTxn)
        })
      }
